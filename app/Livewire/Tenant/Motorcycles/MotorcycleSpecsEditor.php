...
class MotorcycleSpecsEditor extends Component implements HasSchemas
{
    use HasMotorcycleBlockFormState;
    use InteractsWithSchemas;
    use ResolvesMotorcycleRecord;

    private const BLOCK = 'specs';

    /** @var list<string> */
    private const KEYS = ['engine_cc', 'power', 'transmission', 'year', 'mileage', 'specs_json'];
    ...
    public function save(): void
    {
        ...
        try {
            ...
            $m->update($data);
            $this->initialSnapshot = $this->computeSnapshot();
            Notification::make()->title('Характеристики сохранены')->success()->send();
    ...
    {
        return view('livewire.tenant.motorcycles.motorcycle-specs-editor');
    }
}
