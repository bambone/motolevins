...
final class DementievAdvocateBootstrap
{
    ...
    private static function applyBrandingSettings(int $tenantId): void
    {
        ...
        } elseif ($ts->existsPublic($faviconFallbackRel)) {
        ...
        TenantSetting::setForTenant($tenantId, 'seo.indexing_enabled', true, 'boolean');
        TenantSetting::setForTenant($tenantId, 'seo.sitemap_enabled', true, 'boolean');
        TenantSetting::setForTenant($tenantId, 'seo.robots_include_sitemap', true, 'boolean');
    ...
    private static function sectionsCriminal(int $tenantId, $now): callable
    {
        $html = '<p>Уголовная защита: сопровождение на стадии проверки сообщения о преступлении, досудебного производства и судебного разбирательства.</p>'
            ...
            .'<p>На практике важны сроки обжалования процессуальных решений и аккуратная фиксация позиции защиты; при необходимости подключаем экспертизы и иные доказательства в рамках УПК РФ.</p>'
            .'<p>Каждое дело уникально: без гарантий результата, с опорой на факты и процессуальное право. Первичную оценку ситуации можно получить через <a href="/contacts">контакты</a> или <a href="/#expert-inquiry">форму на главной</a>.</p>';
    ...
    private static function sectionsCivil(int $tenantId, $now): callable
    {
        $html = '<p>Гражданские споры: семейные, земельные, жилищные, договорные и иные категории. Претензионный этап, подготовка иска или возражений, представительство в суде.</p>'
            .'<p>Мы заранее оцениваем перспективу и процессуальные риски, готовим доказательственную базу и ведём переговоры там, где это ускоряет решение без затягивания процесса.</p>'
            .'<p>Направления пересекаются с <a href="/practice-areas">общим обзором практик</a>; для записи используйте <a href="/contacts">страницу контактов</a>.</p>';
    ...
    {
        $html = '<p>Арбитражные споры: корпоративные конфликты, сделки, взыскание, взаимодействие с контрагентами и госорганами в рамках Арбитражного процессуального кодекса.</p>'
            .'<p>Сопровождаем досудебную работу (претензии, переписка), подготовку процессуальных документов и реализацию судебных актов. Учитываем отраслевую специфику и документооборот контрагента.</p>'
            .'<p>Обсудить ситуацию можно через <a href="/#expert-inquiry">форму заявки</a> или по реквизитам на <a href="/contacts">странице контактов</a>.</p>';
    ...
    private static function sectionsMigration(int $tenantId, $now): callable
    {
        $html = '<p>Сопровождение иностранных граждан: миграционный учёт, статус, конфликтные ситуации с органами и третьими лицами, процессуальная поддержка.</p>'
            .'<p>Помогаем с подготовкой обращений, жалоб и процессуальных документов, разбором отказов и штрафных решений; по возможности — в досудебном порядке.</p>'
            .'<p>См. также раздел <a href="/practice-areas">«Практики»</a> и <a href="/faq">вопросы до консультации</a>.</p>';
    ...
    private static function seedSeoRecords(int $tenantId, $now): void
    {
        ...
        $graph = [
            ...
            [
                '@type' => 'LegalService',
                'name' => 'Адвокатские услуги — Дементьев Н. В.',
                'areaServed' => [
                    '@type' => 'City',
                    'name' => 'Челябинск',
                ],
        ...
        SeoMeta::withoutGlobalScope('tenant')->updateOrCreate(
            ...
            [
                'meta_title' => 'Адвокат Дементьев Н. В. — Челябинск | уголовные и гражданские дела',
                'meta_description' => 'Адвокат АП Челябинской области (рег. № 74/1764). Уголовная и гражданская защита, арбитраж, суд присяжных, миграция. Запись на консультацию в Челябинске.',
                ...
                'og_description' => 'Защита по уголовным, гражданским и арбитражным делам; суд присяжных. Личный приём в Челябинске по договорённости.',
                'og_image' => null,
                'og_type' => 'website',
        ...
        foreach ($pages as $p) {
            ...
            [$title, $desc, $h1, $ogTitle, $ogDesc] = match ($slug) {
                'about' => [
                    'Об адвокате Дементьеве Н. В. — опыт и направления | Челябинск',
                    'Биография, реквизиты и ключевые направления практики адвоката Дементьева Никиты Владимировича в Челябинске: уголовные дела, гражданский процесс, арбитраж.',
                    $name,
                    'Об адвокате — Дементьев Н. В., Челябинск',
                    'Кто ведёт приём, какие дела берёт в работу и как устроена первичная консультация.',
                ],
                'practice-areas' => [
                    'Практики адвоката в Челябинске — уголовное, гражданское, арбитраж',
                    'Обзор направлений: уголовная защита, суд присяжных, гражданские споры, арбитраж, сопровождение иностранных граждан. С чего начать и как записаться.',
                    'Практики',
                    'Практики адвоката Дементьева — Челябинск',
                    'Уголовная защита, суд присяжных, гражданские и арбитражные споры, миграционные вопросы — кратко о формате работы.',
                ],
                'jury-trial' => [
                    'Суд присяжных Челябинск — адвокат Дементьев Н. В.',
                    'Подготовка к суду с участием коллегии присяжных: стратегия защиты, работа с доказательствами и сопровождение процесса в Челябинске и регионе.',
                    $name,
                    'Суд присяжных — защита адвоката Дементьева',
                    'Сопровождение уголовных дел с присяжными: от предварительного анализа до прений.',
                ],
                'criminal-defense' => [
                    'Уголовный адвокат Челябинск — защита на следствии и в суде',
                    'Помощь подозреваемым и обвиняемым: очные консультации, участие в следственных действиях, апелляции. Опыт работы в Челябинской области.',
                    $name,
                    'Уголовная защита — адвокат Дементьев',
                    'Сопровождение уголовного дела от первого контакта до решения суда; конфиденциальность и понятный план действий.',
                ],
                'civil-disputes' => [
                    'Гражданские споры Челябинск — представительство в суде',
                    'Взыскание долгов, защита прав потребителей, жилищные и семейные конфликты. Подготовка процессуальных документов и ведение дела.',
                    $name,
                    'Гражданские дела — адвокат в Челябинске',
                    'Анализ перспектив, претензионный порядок и судебное представительство по гражданским спорам.',
                ],
                'arbitration' => [
                    'Арбитраж Челябинск — корпоративные и хозяйственные споры',
                    'Судебное представительство в арбитраже: договорные споры, взыскание, банкротство и сопутствующие процедуры для бизнеса.',
                    $name,
                    'Арбитражные споры — адвокат Дементьев',
                    'Практическая помощь компаниям и ИП в арбитражном процессе: от претензии до исполнения решения.',
                ],
                'migration' => [
                    'Миграция и иностранные граждане — юрист в Челябинске',
                    'Визы, разрешения, штрафы и отказы миграционных органов: разбор ситуации, подготовка жалоб и сопровождение по делу.',
                    $name,
                    'Миграционные вопросы — консультация адвоката',
                    'Помощь иностранным гражданам и работодателям в Челябинске: документы, процедуры и защита в спорах.',
                ],
                'faq' => [
                    'Вопросы и ответы — консультация адвоката Дементьева',
                    'Частые вопросы до записи: сроки, документы, формат консультации, направления практики и порядок согласования гонорара.',
                    $name,
                    'FAQ — адвокат Дементьев, Челябинск',
                    'Коротко о том, как проходит первый контакт и чего ожидать от работы с адвокатом.',
                ],
                'contacts' => [
                    'Контакты адвоката Дементьева — Челябинск, офис и связь',
                    'Телефон, email, ВКонтакте и адрес офиса на ул. Братьев Кашириных. Форма обращения; очный приём по предварительной договорённости.',
                    $name,
                    'Связаться с адвокатом — Челябинск',
                    'Как добраться до офиса и отправить запрос онлайн; ответ в рабочее время.',
                ],
                'practice' => [
                    'Подход к делам — как работает адвокат Дементьев',
                    'Принципы ведения дел: анализ рисков, прозрачные этапы, документооборот и коммуникация с клиентом на каждом шаге.',
                    $name,
                    'Метод работы адвоката Дементьева',
                    'Понятная схема сопровождения: от первичной оценки до завершения спора или процесса.',
                ],
                'jurylab-project' => [
                    'Проект JuryLab — образовательный контекст адвоката',
                    'Информация о проекте JuryLab и связи с практикой уголовного процесса и судов присяжных.',
                    $name,
                    'JuryLab и суд присяжных',
                    'Как научный и просветительский контекст пересекается с реальной судебной практикой.',
                ],
                'privacy-policy' => [
                    'Политика конфиденциальности — сайт адвоката Дементьева',
                    'Как обрабатываются персональные данные посетителей сайта и заявок; цели хранения и способы связи по вопросам privacy.',
                    $name,
                    'Конфиденциальность на сайте адвоката',
                    'Текст политики обработки данных для пользователей витрины.',
                ],
                'consent-personal-data' => [
                    'Согласие на обработку персональных данных — форма сайта',
                    'Типовое согласие для отправки заявок и обратной связи через формы на сайте адвоката.',
                    $name,
                    'Согласие на обработку ПДн',
                    'Условия, на которых вы соглашаетесь на обработку данных при обращении через сайт.',
                ],
                default => [
                    $name.' — адвокат Дементьев | Челябинск',
                    'Юридическое сопровождение и представительство адвоката Дементьева Никиты Владимировича в Челябинской области.',
                    $name,
                    $name.' — Челябинск',
                    'Консультации и ведение дел: уточните ситуацию через контакты или форму на сайте.',
                ],
            };

            $pageJsonLd = [];
            if ($slug === 'faq') {
                $faqRows = DB::table('faqs')
                    ->where('tenant_id', $tenantId)
                ...
                if ($faqRows->isNotEmpty()) {
                    $mainEntity = [];
                    foreach ($faqRows as $fr) {
                        $plain = trim(preg_replace('/\s+/u', ' ', strip_tags((string) $fr->answer)) ?? '');
                        if ($plain === '') {
                            continue;
                        }
                        $mainEntity[] = [
                            '@type' => 'Question',
                            'name' => (string) $fr->question,
                            'acceptedAnswer' => [
                                '@type' => 'Answer',
                                'text' => $plain,
                            ],
