...
- **`mkdir(): Permission denied`** (в т.ч. в `TenantMediaBackfillFromR2Command`): пользователь, от которого крутится `php artisan` (Actions SSH при `MEDIA_BACKFILL_AS_WWW_DATA=0` — это deploy-юзер), **не может создавать каталоги** в `MEDIA_LOCAL_ROOT` (часто не хватает прав на новый `tenants/…/public/…` под путь в R2). Варианты: выдать запись в зеркало (группа `www-data`, ACL, `chown`), либо поставить **`MEDIA_BACKFILL_AS_WWW_DATA=1`** в переменных репозитория и настроить passwordless `sudo -u www-data` для SSH-пользователя, если зеркалом владеет `www-data`.
