...
@php
    ...
    $hideSecondaryJumpToForm = ! empty($advocateOnContactsPage ?? false)
        && $btn2 !== ''
        && (
            str_contains($url2, '#contact-inquiry')
            || str_contains($url2, '#expert-inquiry')
        );
@endphp
<section
    class="advocate-contacts-outro-light mb-5 w-full min-w-0 overflow-hidden rounded-xl border border-[rgba(28,31,38,0.08)] bg-white/90 px-4 py-4 shadow-[0_10px_28px_-16px_rgba(28,31,38,0.1)] sm:rounded-[1.25rem] sm:px-6 sm:py-5"
    data-page-section-type="{{ $section->section_type ?? '' }}"
>
    @if ($heading !== '')
        <h2 class="font-serif text-lg font-semibold tracking-tight text-[rgb(24_27_32)] sm:text-xl">{{ $heading }}</h2>
    @endif
    @if ($body !== '')
        <p class="mt-2 max-w-3xl text-pretty text-[14px] leading-relaxed text-[rgb(82_88_99)] sm:text-[15px]">{{ $body }}</p>
    @endif
    @if ($btn !== '' || $btn2 !== '')
        <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
            @if ($btn !== '')
                ...
                >{{ $btn }}</a>
            @endif
            @if ($btn2 !== '' && ! $hideSecondaryJumpToForm)
                <a
                    href="{{ e($url2) }}"
