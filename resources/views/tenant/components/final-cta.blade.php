@props([
    'section' => null,
    /** wa.me/…: null = секция/сервис; '' = отключить WA (без fallback). */
    'whatsAppUrl' => null,
...
@php
    ...
    $d = is_array($section) ? $section : [];
    $heading = $d['heading'] ?? 'Готовы обсудить детали?';
    $description = $d['description'] ?? 'Оставьте заявку на сайте или выберите удобный способ связи — ответим в рабочее время.';
    $buttonText = $d['button_text'] ?? 'Связаться';
    $headingTied = RussianTypography::tiePrepositionsToNextWord($heading);
    $descriptionTied = RussianTypography::tiePrepositionsToNextWord($description);
    $tenant = tenant();
    $secondaryWaHref = FinalCtaWhatsappUrl::resolve(
        $whatsAppUrl,
        (string) ($d['whatsapp_url'] ?? ''),
        (bool) $showWhatsAppSecondary,
        $tenant instanceof Tenant ? $tenant : null,
        app(TenantPublicSiteContactsService::class),
    );
    $primaryAction = (string) ($d['primary_action'] ?? 'link');
    $primaryLinkHref = trim((string) ($d['button_url'] ?? $d['primary_href'] ?? ''));
    if ($primaryLinkHref === '') {
        $primaryLinkHref = (string) route('contacts');
    }
    $bgPath = trim((string) ($d['background_image'] ?? ''));
    $ctaBackgroundUrl = $bgPath !== ''
        ? (str_starts_with($bgPath, 'http://') || str_starts_with($bgPath, 'https://') ? $bgPath : theme_platform_asset_url($bgPath))
        : theme_platform_asset_url('marketing/hero-bg.png');
    // data_json.logo_image: нет ключа — дефолтный круглый логотип; ключ задан, значение null/""/только пробелы — скрыть img;
    // иначе путь к platform-asset (theme) или полный http(s) URL.
    if (! array_key_exists('logo_image', $d)) {
        $ctaLogoUrl = theme_platform_asset_url('marketing/logo-round-dark.png');
    } else {
        $rawLogo = $d['logo_image'] ?? null;
        $logoTrim = is_string($rawLogo) ? trim($rawLogo) : trim((string) $rawLogo);
        if ($rawLogo === null || $logoTrim === '') {
            $ctaLogoUrl = null;
        } elseif (str_starts_with($logoTrim, 'http://') || str_starts_with($logoTrim, 'https://')) {
            $ctaLogoUrl = $logoTrim;
        } else {
            $ctaLogoUrl = theme_platform_asset_url($logoTrim);
        }
    }
@endphp
<section class="relative z-10 overflow-hidden border-t border-white/[0.02] pt-24 pb-20 sm:pt-28 sm:pb-24 md:pt-36 md:pb-28 lg:pt-40 lg:pb-32" aria-labelledby="final-cta-heading">
    ...
    <div class="pointer-events-none absolute inset-0 z-0 flex items-center justify-center">
        <div class="h-[280px] w-[280px] rounded-full bg-amber-500/10 blur-[100px] sm:h-[420px] sm:w-[420px] sm:blur-[120px]"></div>
    </div>

    <div class="relative z-10 mx-auto max-w-4xl px-3 text-center sm:px-4 md:px-8">
        @if ($ctaLogoUrl !== null)
        <img src="{{ e($ctaLogoUrl) }}" alt="" class="mx-auto mb-6 h-12 w-12 object-contain opacity-95 sm:mb-8 sm:h-14 sm:w-14" width="56" height="56" loading="lazy" decoding="async" fetchpriority="low" />
        @endif
        ...
        <div class="flex flex-col items-stretch justify-center gap-4 sm:flex-row sm:items-center sm:gap-5">
            @if ($primaryAction === 'scroll_catalog')
            ...
            @else
            <a href="{{ e($primaryLinkHref) }}" class="tenant-btn-primary inline-flex min-h-12 w-full items-center justify-center px-8 py-3.5 text-base shadow-lg transition-all duration-300 hover:-translate-y-0.5 sm:w-auto sm:min-h-12 sm:px-8 sm:py-3.5 sm:text-base">
                {{ $buttonText }}
            </a>
            @endif

            @if ($secondaryWaHref !== '')
            <a href="{{ e($secondaryWaHref) }}" target="_blank" rel="noopener noreferrer" class="tenant-btn-secondary min-h-12 w-full gap-3 px-8 py-3.5 text-base font-semibold shadow-md backdrop-blur-md transition-all duration-300 sm:w-auto sm:min-h-12 sm:px-8 sm:py-3.5 sm:text-base">
