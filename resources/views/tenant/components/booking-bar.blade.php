    ...
    <div class="bg-black/25 backdrop-blur-md border border-white/10 rounded-2xl p-4 md:p-5">
        <div class="flex flex-col lg:flex-row gap-3 lg:gap-4 lg:items-end">
            ...
            <div class="flex-1 min-w-0">
                ...
                <input type="date" id="start_date" name="rental_start_date" x-model="filters.start_date" required min="{{ date('Y-m-d') }}" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white text-base focus:bg-black/60 focus:ring-2 focus:ring-moto-amber/40 focus:border-moto-amber/30 outline-none transition-all hover:border-white/20 h-12 [color-scheme:dark]">
            </div>

            <div class="flex-1 min-w-0">
                <label for="end_date" class="block text-xs font-semibold text-white/60 mb-1.5 uppercase tracking-widest">По</label>
                <input type="date" id="end_date" name="rental_end_date" x-model="filters.end_date" required min="{{ date('Y-m-d') }}" autocomplete="off" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 text-white text-base focus:bg-black/60 focus:ring-2 focus:ring-moto-amber/40 focus:border-moto-amber/30 outline-none transition-all hover:border-white/20 h-12 [color-scheme:dark]">
            </div>

            <div class="lg:shrink-0">
                {{-- Spacer for lg alignment with labeled fields; not a form label (avoids a11y "label without control"). --}}
                <div class="hidden lg:block text-xs mb-1.5 opacity-0 pointer-events-none select-none" aria-hidden="true">&nbsp;</div>
                <button type="submit"
                        :disabled="isSearching"
                        :class="isSearching ? 'opacity-70 cursor-not-allowed' : 'hover:-translate-y-0.5 hover:shadow-xl hover:shadow-moto-amber/35 active:scale-[0.97]'"
                        class="tenant-btn-primary w-full lg:w-auto min-h-12 px-8 text-base whitespace-nowrap">
