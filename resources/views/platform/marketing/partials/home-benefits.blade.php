...
<section id="vozmozhnosti" class="pm-section-anchor pm-section-y border-b border-slate-200 bg-slate-50" aria-labelledby="vozmozhnosti-heading">
    <div class="relative z-10 mx-auto max-w-6xl px-4 md:px-6">
        <h2 id="vozmozhnosti-heading" class="fade-reveal text-balance text-3xl font-extrabold leading-tight text-slate-900 sm:text-4xl">
            {{ RussianTypography::tiePrepositionsToNextWord('Это не набор функций. Это готовый процесс.') }}
        </h2>
        <p class="fade-reveal pm-section-lead max-w-2xl text-pretty text-base leading-relaxed text-slate-700 sm:text-lg [transition-delay:100ms]">
            {{ RussianTypography::tiePrepositionsToNextWord('Пока другие дают инструменты для сборки, RentBase даёт фундамент, на котором бизнес начинает работать сразу.') }}
        </p>
        ...
        <div class="fade-reveal mt-6 grid grid-cols-1 gap-4 md:mt-8 md:grid-cols-2 md:gap-5 lg:grid-cols-3 [transition-delay:150ms]">
            ...
            <article class="pm-benefit-card group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200/80 hover:bg-emerald-50/25 hover:shadow-xl active:scale-[0.99] sm:p-8">
                ...
                <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">{{ RussianTypography::tiePrepositionsToNextWord('Всё уже связано') }}</h3>
                <p class="mt-3 text-pretty text-xs font-semibold uppercase tracking-wide text-slate-500">Заявка → CRM → бронь → уведомления</p>
                <p class="mt-3 text-pretty text-sm leading-relaxed text-slate-700 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Данные идут по системе сами — без ручной склейки и потерь.') }}</p>
            </article>
            ...
            <article class="pm-benefit-card group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200/80 hover:bg-emerald-50/25 hover:shadow-xl active:scale-[0.99] sm:p-8">
                ...
                <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">{{ RussianTypography::tiePrepositionsToNextWord('Настоящее бронирование') }}</h3>
                <p class="mt-3 text-pretty text-xs font-semibold uppercase tracking-wide text-slate-500">Слоты, конфликты, статусы</p>
                <p class="mt-3 text-pretty text-sm leading-relaxed text-slate-700 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Проверка доступности и статусы — без ошибок в календаре.') }}</p>
            </article>
            ...
            <article class="pm-benefit-card group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200/80 hover:bg-emerald-50/25 hover:shadow-xl active:scale-[0.99] sm:p-8">
                ...
                <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">{{ RussianTypography::tiePrepositionsToNextWord('CRM для операций') }}</h3>
                <p class="mt-3 text-pretty text-xs font-semibold uppercase tracking-wide text-slate-500">История, задачи, сегменты</p>
                <p class="mt-3 text-pretty text-sm leading-relaxed text-slate-700 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Не список заявок — центр управления клиентами и статусами.') }}</p>
            </article>
            ...
            <article class="pm-benefit-card group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200/80 hover:bg-emerald-50/25 hover:shadow-xl active:scale-[0.99] sm:p-8">
                ...
                <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">{{ RussianTypography::tiePrepositionsToNextWord('Ваш сайт и домен') }}</h3>
                <p class="mt-3 text-pretty text-xs font-semibold uppercase tracking-wide text-slate-500">SEO, темы, витрина</p>
                <p class="mt-3 text-pretty text-sm leading-relaxed text-slate-700 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Свой домен и нормальная витрина — не чужой поддомен.') }}</p>
            </article>
            ...
            <article class="pm-benefit-card group flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200/80 hover:bg-emerald-50/25 hover:shadow-xl active:scale-[0.99] sm:p-8">
                ...
                <h3 class="text-lg font-bold leading-snug text-slate-900 sm:text-xl">{{ RussianTypography::tiePrepositionsToNextWord('Готово к масштабу') }}</h3>
                <p class="mt-3 text-pretty text-xs font-semibold uppercase tracking-wide text-slate-500">Нагрузка и очереди</p>
                <p class="mt-3 text-pretty text-sm leading-relaxed text-slate-700 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Растёт вместе с бизнесом: ядро, фоновые задачи, хранилище.') }}</p>
            </article>
            ...
            <article class="group flex h-full flex-col items-center justify-center rounded-3xl border-2 border-dashed border-slate-300 bg-white/80 p-7 transition-all duration-300 hover:-translate-y-0.5 hover:border-indigo-400 hover:bg-indigo-50/25 hover:shadow-md active:scale-[0.99] sm:p-8">
                <p class="text-center text-pretty text-sm font-semibold text-slate-600 sm:text-base">{{ RussianTypography::tiePrepositionsToNextWord('Ещё десятки функций под ваш сервис') }}</p>
