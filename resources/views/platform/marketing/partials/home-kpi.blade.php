<section id="statistika" class="pm-section-anchor relative overflow-hidden border-b border-slate-200 bg-slate-50 py-12 sm:py-16 md:py-20" aria-labelledby="statistika-heading">
    ...
    <div class="relative z-10 mx-auto max-w-6xl px-3 sm:px-4 md:px-6">
        <h2 id="statistika-heading" class="fade-reveal text-balance text-xl font-bold leading-tight text-slate-900 sm:text-2xl md:text-3xl">{!! str_replace([' для ', ' с ', ' в ', ' и '], [' для&nbsp;', ' с&nbsp;', ' в&nbsp;', ' и&nbsp;'], $pm['kpi_section_title'] ?? 'Платформа уже работает в бизнесе') !!}</h2>
        <p class="fade-reveal mt-3 text-pretty text-sm leading-relaxed text-slate-600 sm:text-base" style="transition-delay: 100ms;">{!! str_replace([' для ', ' с ', ' в ', ' и ', ' — '], [' для&nbsp;', ' с&nbsp;', ' в&nbsp;', ' и&nbsp;', '&nbsp;— '], $pm['kpi_section_intro'] ?? 'Каждая метрика — про реальную работу системы.') !!}</p>
        <div class="fade-reveal mt-8 grid gap-0 divide-y divide-slate-200 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:mt-10 md:grid-cols-3 md:divide-x md:divide-y-0" style="transition-delay: 200ms;">
            @foreach($pm['kpi'] ?? [] as $i => $row)
                <div class="fade-reveal pm-reveal-kpi-{{ min($i, 2) }} flex flex-col justify-center p-6 sm:p-8">
                    ...
                    @endif
                    <p class="mt-2 text-pretty text-sm leading-relaxed text-slate-600">{!! str_replace([' для ', ' с ', ' в ', ' и ', ' — '], [' для&nbsp;', ' с&nbsp;', ' в&nbsp;', ' и&nbsp;', '&nbsp;— '], $row['why'] ?? '') !!}</p>
                </div>
            @endforeach
        </div>
        <p class="fade-reveal mt-8 text-center text-sm font-medium text-slate-700 sm:text-base" style="transition-delay: 500ms;">{!! str_replace([' для ', ' с ', ' в ', ' и ', ' — '], [' для&nbsp;', ' с&nbsp;', ' в&nbsp;', ' и&nbsp;', '&nbsp;— '], $pm['kpi_section_footer'] ?? 'Это не концепт — это работающая система') !!}</p>
    </div>
</section>
